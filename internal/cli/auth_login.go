...
func runAuthLogin(ctx context.Context, workdir, configFile string) error {
	...
	if err != nil {
		logger.ErrorContext(ctx, "oauth login failed", "mode", "callback", "error", err)
	...
	logger.InfoContext(ctx, "oauth login succeeded", "mode", "callback", "token_path", paths.TokenPath)
